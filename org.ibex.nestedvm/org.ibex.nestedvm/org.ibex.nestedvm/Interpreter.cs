...
namespace org.ibex.nestedvm
{
	...
	public class Interpreter : UnixRuntime //, ICloneable
	{
		...
		private bool FC
		{
			set
			{
				fcsr = (fcsr & ~0x800000) | (value ? 0x800000 : 0x000000);
			}
		}
		private int roundingMode() // bits 0-1
		{
			return fcsr & 3;
		}
...
//ORIGINAL LINE: protected void _execute() throws ExecutionException
		protected internal override  void _execute()
		{
			...
			{
				e.Location = toHex(pc) + ": " + sourceLine(pc);
				throw e;
			}
		}
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: private final int runSome() throws FaultException,ExecutionException
		private int runSome()
		{
...
//ORIGINAL LINE: final int PAGE_WORDS = (1<<pageShift)>>2;
		...
		public virtual int lookupSymbol(string name)
		{
		...
		protected internal virtual int userInfoBae()
		{
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: private void loadImage(Seekable data) throws IOException
      ...
      //TODO: WTF--->
			...
			for (int i = 0;i < pheaders.Length;i++)
			{
				...
				{
					int page = (int)((uint)(j + addr) >> pageShift);
...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public Interpreter(Seekable data) throws IOException
		public Interpreter(Seekable data) : base(4096,65536)
		{
			loadImage(data);
		}
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
...
//    }
      ...
      /*
			...
			try
			{
				Process p = java.lang.Runtime.Runtime.exec(new string[]{addr2line,"-e",image,toHex(pc)});
				line = (new BufferedReader(new InputStreamReader(p.InputStream))).readLine();
				if (line == null)
				{
					return null;
				}
				...
				{
					sourceLineCache = new Hashtable();
				}
				sourceLineCache[new int?(pc)] = line;
				return line;
			}
			catch (IOException e)
			{
		...
		public class DebugShutdownHook //: Runnable
		{
			...
			public DebugShutdownHook(Interpreter outerInstance)
			{
